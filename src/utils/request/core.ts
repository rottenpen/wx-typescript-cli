...
class Core {
  ...
  [x: string]: any;
  public instanceRequestInterceptors: any[]
  public instanceResponseInterceptors: any[]
  ...
  // 旧版拦截器为共享
  static requestInterceptors:any[] = [addfixInterceptor];
  ...
  dealRequestInterceptors(ctx) {
    ...
    return allInterceptors.reduce(reducer, Promise.resolve()).then((ret:Ret = {}) => {
      ctx.req.url = ret.url || ctx.req.url;
      ctx.req.options = ret.options || ctx.req.options;
